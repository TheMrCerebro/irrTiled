        ...
        for (u32 j = 0; j < tld->Layer.size(); ++j)
        {
            ...
            for (u32 d = 0; d < lyr.Data.size(); ++d)
            {
                ...
                if (id >= 0)
                {
                    ...
                    IMeshSceneNode* mesh = smgr->addCubeSceneNode(ts.Size.Width, 0, -1, vector3df(-x, j*(ts.Size.Width), y));
    ...
    for (u32 i = 0; i < tld->ObjectGroup.size(); ++i)
    {
        ...
        for (u32 j = 0; j<og.Object.size(); ++j)
        {
            ...
            else
            {
                ...
                scene::IBillboardSceneNode* sprite = smgr->addBillboardSceneNode(0, dimension2df(tld->TileSize.Width, tld->TileSize.Width), vector3df(-x, 64.0f, y));
